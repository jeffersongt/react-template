...
async function searchCompany(id: string) {
  ...
  const result: Company = { id: "", name: "", domain: "", adress: "" };
...
async function getProfile() {
  const url = "http://localhost:8000/users/me/profiles";
  const profile: Infos = {
    id: "",
    first_name: "",
    last_name: "",
...
async function getExperience() {
  const url = "http://localhost:8000/users/me/experiences";
...
async function postExperience(params: {
  position: string;
  company: string;
  city: string;
}) {
  ...
  await axios
    ...
    .then((res) => {
      console.log(res);
      alert(
        "L'expérience " + res.data.position + " a été ajoutée avec succès !"
      );
    })
    .catch(function (error) {
      if (error.response) {
...
async function patchExperience(
  params: { position: string; company: string; city: string },
  id: string
) {
...
async function getCompetence() {
  const url = "http://localhost:8000/users/me/competences";
  const competence: Competences = { id: [], competence: [] };
...
async function getCompany() {
  const url = "http://localhost:8000/users/me/companies";
  const company: Companies = { id: [], name: [], domain: [], adress: [] };
...
async function postCompany(parameters: {
  name: string;
  domain: string;
  adress: string;
}) {
...
async function patchCompany(
  parameters: { name: string; domain: string; adress: string },
  id: string
) {
...
async function getEmployee(id: string) {
  const url: string =
    "http://localhost:8000/users/me/companies/" + id + "/employees";
  const arrEmployee: { id: string[]; userId: string[]; userName: string[] } = {
    id: [],
    userId: [],
    userName: [],
  };
...
async function deleteEmployee(companyId: string, employeeId: string) {
  const url: string =
    "http://localhost:8000/users/me/companies/" +
    companyId +
    "/employees/" +
    employeeId;
